...
procedure SQLite3ConsoleTests(var aText:Text);
...
type
  ...
  TTestSynopsemORMotFramework = class(TSynTests)
  public
...
begin
  ...
  try
    {$ifdef Android}
    ExeVersion.ProgramFilePath:=BaseStorageDirectory;
    TSQLLog.Family.DestinationPath:=BaseStorageDirectory;
    SaveToText(aText);
    {$endif}
    if ParamCount<>0 then
    begin
      //SaveToFile('Yolo.dat'); // DestPath on command line -> export to file
