...
const
  ...
  HTTP_DEFAULTPORT = '8448'; // under Linux, port<1024 needs root user

  {$endif MSWINDOWS}

var
  BaseStorageDirectory:string='C:\Users\Alfred\Documents\GitHub\mORMot\SQLite3\fpc\bin\i386-win32\files\';
...
begin
  ...
  Check(FileFromString(Content,BaseStorageDirectory+'test.ini'),'test.ini');
  Check(FileSynLZ(BaseStorageDirectory+'test.ini',BaseStorageDirectory+'test.ini.synlz',$ABA51051),'synLZ');
  if CheckFailed(FileUnSynLZ(BaseStorageDirectory+'test.ini.synlz',BaseStorageDirectory+'test2.ini',$ABA51051),'unSynLZ') then
    exit;
  S := StringFromFile(BaseStorageDirectory+'test2.ini');
...
begin
  ...
  try // with hash table
    ...
    L.SaveToFile(BaseStorageDirectory+'utf8list.txt');
    L.Clear;
    Check(L.Count=0);
    L.LoadFromFile(BaseStorageDirectory+'utf8list.txt');
    Check(L.Count=n);
    for i := 1 to MAX do begin
      UInt32ToUTF8(i,s);
      Check((L.IndexOf(s)>=0)=(i and 127<>0));
...
begin
  ...
  try
    ...
    Test;
    FN := BaseStorageDirectory+ChangeFileExt(ExtractFileName(ExeVersion.ProgramFileName),'.syntable');
    DeleteFile(FN);
...
begin
  ...
  for spec := 0 to High(MUSTACHE_SPECS) do begin
    mustacheJsonFileName := BaseStorageDirectory+MUSTACHE_SPECS[spec]+'.json';
    ...
    if mustacheJson='' then begin
      mustacheJson := HttpGet('https://raw.githubusercontent.com/mustache/spec/'+
        'master/specs/'+StringToAnsi7(ExtractFileName(mustacheJsonFileName)));
...
{$ifndef LVCL}
  ...
  begin
    ...
    if soWriteHumanReadable in Options then
      FileFromString(U,BaseStorageDirectory+'zendframeworkSaved.json');
    Check(length(git)>=30);
    Check(length(U)>3000);
...
{$endif LVCL}
  ...
  discogsJson := StringFromFile(BaseStorageDirectory+discogsFileName);
  if discogsJson='' then begin
    discogsJson := HttpGet('https://api.discogs.com/artists/45/releases?page=1&per_page=100');
    FileFromString(discogsJson,BaseStorageDirectory+discogsFileName);
  end;
  Check(IsValidJSON(discogsJson));
  zendframeworkJson := StringFromFile(BaseStorageDirectory+zendframeworkFileName);
  if zendframeworkJson='' then begin
    zendframeworkJson := HttpGet('https://api.github.com/users/zendframework/repos');
    FileFromString(zendframeworkJson,BaseStorageDirectory+zendframeworkFileName);
  end;
  ...
  FileFromString(U,BaseStorageDirectory+'transactions.json');
  TTextWriter.RegisterCustomJSONSerializerFromText(TypeInfo(TTestCustomJSON2Title),'');
  TTextWriter.RegisterCustomJSONSerializerFromText(TypeInfo(TTestCustomJSON2),'');
  ...
  U := RecordSaveJSON(Disco,TypeInfo(TTestCustomDiscogs));
  Check(IsValidJSON(U));
  FileFromString(U,BaseStorageDirectory+'discoExtract.json');
  Finalize(Disco);
  FillCharFast(Disco,sizeof(Disco),0);
...
begin
  ...
  if discogs<>'' then begin
    FileFromString(JSONReformat(discogs),ChangeFileExt(BaseStorageDirectory+discogsFileName,'2.json'));
...
begin
  ...
  discogs := StringFromFile(BaseStorageDirectory+discogsFileName);
  CheckNestedDoc([]);
  ...
  {$ifdef FPC}_Safe(V1._(1))^{$else}V1._(1){$endif}.Delete('year');
  Check(V1{$ifdef FPC}._JSON{$endif}='["root",{"name":"Jim"},3.1415]');
  {$ifdef FPC}_Safe(V1)^{$else}V1{$endif}.Delete(1);
...
begin
  ...
  try
    DeleteFile(BaseStorageDirectory+'fullmem.data');
    Check(not FileExists(BaseStorageDirectory+'fullmem.data'));
    Server := TSQLRestServerFullMemory.Create(Model,BaseStorageDirectory+'fullmem.data',true,true);
    try
      ...
      try
        ...
        try
          ...
          Check(FileExists(BaseStorageDirectory+'fullmem.data'));
...
begin
  ...
  try
    ...
    tmpFN := BaseStorageDirectory+'TestSQL3zipformat.tmp';
...
begin
  ...
  FN := BaseStorageDirectory+ChangeFileExt(ExeName,'.zip');
  ...
  {$endif}
  DeleteFile(FN);
  FN2 := BaseStorageDirectory+'ddd.zip';
  ...
  try
    ...
    if not DirectoryExists(FN) then
      FN := ExeVersion.ProgramFilePath+'..'+PathDelim+'..'+PathDelim+'DDD';
    if not DirectoryExists(FN) then
      FN := ExeVersion.ProgramFilePath+'..'+PathDelim+'..'+PathDelim+'..'+PathDelim+'DDD';
    if not DirectoryExists(FN) then
      FN := ExeVersion.ProgramFilePath+'..'+PathDelim+'..'+PathDelim+'..'+PathDelim+'..'+PathDelim+'DDD';
...
procedure TTestCompression._TAlgoCompress;
  ...
  begin
    ...
    timecomp := 0;
    timedecomp := 0;
    log := StringFromFile(BaseStorageDirectory+'bigTest.log');
    for i := 0 to 100 do begin
      if log<>'' then
        s := log else
      ...
      if (log<>'') and (s2<>s) then FileFromString(s2,BaseStorageDirectory+'bigTest'+algo.ClassName+'.log');
      inc(plain, length(s));
      inc(comp, length(t));
      if log<>'' then
        break;
    ...
    Check(s2=s, algo.ClassName);
    if (log<>'') and (s2<>s) then FileFromString(s2,BaseStorageDirectory+'bigTestPartial'+algo.ClassName+'.log');
  end;
begin
  TestAlgo(AlgoSynLZ);
...
begin
  //If dddInfraApps is not included in the uses clause, this Register is not run
  //So do it here, no harm caused
  TJSONSerializer.RegisterObjArrayForJSON(
...
//  FileFromString(sav,'privkey.pas');
    ...
    secret.Free;
    cert.Free;
    check(selfsignedroot.SaveToSecureFile('pass',BaseStorageDirectory,64,1000));
    ...
    chain.SaveToFile(BaseStorageDirectory+'test');
    {$endif}
    ...
    BaseStorageDirectory,serial,'pass',1000);
    {$ifndef DELPHI5OROLDER}
    check(chain.LoadFromFile(BaseStorageDirectory+'test'));
    check(chain.Count=3);
    check(chain.IsValid(selfsignedroot)=ecvValidSelfSigned);
    ...
    {$endif}
    sav:=BaseStorageDirectory+selfsignedroot.SaveToSecureFileName;
    check(DeleteFile(sav));
    selfsignedroot.Free;
  finally
    chain.Free;
  end;
...
begin
  if DirectoryExists(BaseStorageDirectory+'synecc') then
    DirectoryDelete(BaseStorageDirectory+'synecc',FILES_ALL,true) else
    CreateDir(BaseStorageDirectory+'synecc');
  SetCurrentDir(BaseStorageDirectory+'synecc');
  try
    ...
    SetCurrentDir(ExeVersion.ProgramFilePath);
  end;
end;
{$endif}
...
begin
  ...
  if ClassType=TTestMemoryBased then
    ...
    TempFileName := BaseStorageDirectory+'test.db3';
  ...
  FileFromString(JS,BaseStorageDirectory+'Test1.json');
...
begin
  ...
  s:='temptest.log';
  {$else}
  s:=BaseStorageDirectory+'temptest.log';
  {$endif}
  FileFromString(LOG1,s);
  Demo.Execute('CREATE VIRTUAL TABLE test USING log('+s+');');
...
begin
  ...
  Client := TSQLRestClientDB.Create(Model,nil,BaseStorageDirectory+'test.db3',TSQLRestServerDB,false,'');
  try
...
var Model: TSQLModel;
  ...
  begin
    Master := CreateServer(BaseStorageDirectory+'testversion'+DBExt,DeleteDBFile);
...
begin
  ...
  CreateMaster(true);
  Slave1 := CreateServer(BaseStorageDirectory+'testversionreplicated'+DBExt,true);
  Slave2 := CreateServer(BaseStorageDirectory+'testversioncallback'+DBExt,true);
  try
    Rec := TSQLRecordPeopleVersioned.CreateAndFillPrepare(StringFromFile(BaseStorageDirectory+'Test1.json'));
...
begin
  ...
  try
    DataBase := TSQLRestServerDB.Create(Model,BaseStorageDirectory+'test.db3');
...
begin
  ...
  FileFromString(JSONReformat(stats),BaseStorageDirectory+'statsClientServer.json');
  FreeAndNil(Client);
end;
...
var Props: TSQLDBConnectionProperties;
    ...
    n: integer;
begin
  Props := TSQLDBSQLite3ConnectionProperties.Create(BaseStorageDirectory+'test.db3','','','');
  try
...
begin
  ...
  Props.DefinitionToFile(BaseStorageDirectory+'connectionprops.json');
  Props.Free;
  Props := TSQLDBConnectionProperties.CreateFromFile(BaseStorageDirectory+'connectionprops.json');
  ...
  Props.Free;
  DeleteFile(BaseStorageDirectory+'connectionprops.json');
end;
...
{$endif}
begin
  DeleteFile(BaseStorageDirectory+'testpass.db3');
  Model := TSQLModel.Create([TSQLRecordPeople]);
  try
    Client := TSQLRestClientDB.Create(Model,nil,BaseStorageDirectory+'test.db3',TSQLRestServerDB,false,'');
    try
      ...
      R.FillPrepare(fPeopleData);
      try
        Client2 := TSQLRestClientDB.Create(Model,nil,BaseStorageDirectory+'testpass.db3',TSQLRestServerDB,false,password);
        try
        ...
        Check(IsSQLite3File(BaseStorageDirectory+'testpass.db3'));
        Check(IsSQLite3FileEncrypted(BaseStorageDirectory+'testpass.db3')=(password<>''),'encrypt1');
        ...
        Check(IsSQLite3File(BaseStorageDirectory+'testpass.db3'));
        Check(IsSQLite3FileEncrypted(BaseStorageDirectory+'testpass.db3')=(password<>''),'encrypt2');
        {$ifndef NOSQLITE3ENCRYPT}
        // now read it after uncypher
        check(ChangeSQLEncryptTablePassWord(BaseStorageDirectory+'testpass.db3',password,''));
        Check(IsSQLite3File(BaseStorageDirectory+'testpass.db3'));
        Check(not IsSQLite3FileEncrypted(BaseStorageDirectory+'testpass.db3'),'encrypt3');
        Client2 := TSQLRestClientDB.Create(Model,nil,BaseStorageDirectory+'testpass.db3',TSQLRestServerDB,false,'');
        try
          R2 := TSQLRecordPeople.CreateAndFillPrepare(Client2,'');
          try
            CheckFilledRow;
...
begin
  ...
  DeleteFile(BaseStorageDirectory+'extdata.db3');
  fProperties := TSQLDBSQLite3ConnectionProperties.Create(BaseStorageDirectory+'extdata.db3','','','');
  ...
  DeleteFile(BaseStorageDirectory+'testExternal.db3'); // need a file for backup testing

  if TrackChanges and StaticVirtualTableDirect then begin
    DeleteFile(BaseStorageDirectory+'history.db3');
    historyDB := TSQLRestServerDB.Create(
      TSQLModel.Create([TSQLRecordMyHistory],'history'),
      BaseStorageDirectory+'history.db3',false);
  end else
    historyDB := nil;
  aExternalClient := TSQLRestClientDB.Create(fExternalModel,nil,BaseStorageDirectory+'testExternal.db3',TSQLRestServerDB);
  try
  ...
  try
    ...
    try
      ...
      try
        ...
        Client.Server.SessionsSaveToFile(BaseStorageDirectory+'sessions.data');
        Client.Server.SessionsLoadFromFile(BaseStorageDirectory+'sessions.data',false);
        ...
        {$endif}
        // test backup API
        BackupFN := Format(BaseStorageDirectory+'backupbackground%s.dbsynlz',[ClassName]);
        deleteFile(BackupFN);
        BackupTimer.Start;
        ...
        if ClassType=TTestMemoryBased then begin // time consuming, so do it once
          ...
          try
            ...
            DeleteFile(BaseStorageDirectory+'People.json');
            DeleteFile(BaseStorageDirectory+'People.data');
            Server.StaticDataCreate(TSQLRecordPeople,BaseStorageDirectory+'People.data',true);
            ...
            Check(V.FirstName='first');
            aStatic.FileName := BaseStorageDirectory+'People.json';
...
begin
  ...
  fClient := TSQLRestClientDB.Create(fModel,nil,BaseStorageDirectory+'test.db3',TSQLRestServerDB,true);
...
begin
  ...
  {$endif}
  {$endif}
  DeleteFile(BaseStorageDirectory+'servicelog.db');
  Log := TSQLRestServerDB.CreateWithOwnModel([TSQLRecordServiceLog],BaseStorageDirectory+'servicelog.db');
  try
...
begin
  ...
  store := TSQLRestServerDB.CreateWithOwnModel([TSQLMonitorUsage],BaseStorageDirectory+'servicestats.db3');
  try
    ...
    JSONReformatToFile(stats,BaseStorageDirectory+'statsSessions.json');
...
begin
  ...
  if fClientOnlyServerIP='' then begin
    DeleteFile(BaseStorageDirectory+TTESTMULTITHREADPROCESS_DBFILENAME);
    if CheckFailed(not FileExists(BaseStorageDirectory+TTESTMULTITHREADPROCESS_DBFILENAME)) or
       CheckFailed(aClass<>nil) then
      exit;
    fDatabase := TSQLRestServerDB.Create(fModel,BaseStorageDirectory+TTESTMULTITHREADPROCESS_DBFILENAME);
...
begin
  if FileExists(BaseStorageDirectory+ChangeFileExt(ExtractFileName(ExeVersion.ProgramFileName),'.db3')) then
    SysUtils.DeleteFile(BaseStorageDirectory+ChangeFileExt(ExtractFileName(ExeVersion.ProgramFileName),'.db3'));
  CheckNot(FileExists(BaseStorageDirectory+ChangeFileExt(ExtractFileName(ExeVersion.ProgramFileName),'.db3')));
end;

procedure TTestDDDMultiThread.StartServer;
begin
  fRestServer := TSQLRestServerDB.CreateWithOwnModel([TSQLRecordDDDTest], BaseStorageDirectory+ChangeFileExt(ExtractFileName(ExeVersion.ProgramFileName),'.db3'), true);
