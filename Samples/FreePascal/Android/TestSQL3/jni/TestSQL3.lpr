...
uses
  ...
  PXL.Logs,
  {$ENDIF}
  PXL.TypeDef, PXL.Types, PXL.Devices, PXL.Canvas, PXL.Images, PXL.Fonts,
...
begin
  ...
  begin
    ...
    RedirectFile(StdOut,NewList);
    RedirectFile(ErrOutput,NewList);
    RedirectFile(StdErr,NewList);
  end
  else
  begin
    SysInitStdIO;
...
begin
  ...
  //BaseStorageDirectory:=StrPas(Application.AndroidApp^.activity^.externalDataPath)+'/';
  BaseStorageDirectory:=StrPas(Application.AndroidApp^.activity^.internalDataPath)+'/';
  mORMotResult.Append('Welcome !');
  mORMotResult.Append('This is mORMot on Android.');
